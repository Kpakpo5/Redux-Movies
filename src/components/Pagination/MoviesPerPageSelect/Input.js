...
const Input = ({ inputId, inputValue}) => {
    ...
    return (
        <div className="flex items-center text-yellow-400 mx-6 cursor-pointer">
            <input checked={moviesPerPage === inputValue} onChange={() => dispatch(setMoviesPerPage(inputValue))} id={inputId} type="radio" value={inputValue.toString()} name="per-page"          className="cursor-pointer w-4 h-4 text-yellow-400 border-gray-300 accent-yellow-400" />
